<template>
  ...
  <div class="app-container">
    ...
    <el-form-item>
        <el-select v-model="SnSearchData"
                   @change="function1"
    ...
    <el-form-item>
        <el-select v-model="TypeSearchData"
                  @change="function2"
                   placeholder="请选择设备类型">
          <el-option
            v-for="item in deviceList"
    ...
    <el-form-item>
        <el-select v-model="MmacSearchData"
                   @change="function3"
                   placeholder="请选择MAC">
          <el-option
            v-for="item in deviceList"
    ...
    <el-form-item>
        <el-select v-model="MipSearchData"
                   @change="function4"
                   placeholder="请选择IP">
          <el-option
            v-for="item in deviceList"
    ...
    .catch(_ => {});
      ...
      getDeviceData(){//获取数据
        getDeviceList().then((response) => {
          ...
          //this.deviceTypeList=this.deviceData.type;
          //this.deviceMmacList=this.deviceData.mmac;
          //this.deviceMipList=this.deviceData.mip;
        });
      },
      addDeviceData(data){//增
       ...
       deleteDeviceData(id) {
        ...
        //实现筛选的函数
        function1(){
          this.deviceData= this.searchSn(this.SnSearchData);
        },
        searchSn(keywords) {
        ...
        function2(){
          this.deviceData= this.searchType(this.TypeSearchData);
        },
        searchType(keywords) {
          return this.deviceData.filter(item =>{
        ...
        function3(){
          this.deviceData= this.searchMmac(this.MmacSearchData);
        },
        searchMmac(keywords) {
          return this.deviceData.filter(item =>{
            if(item.mmac==keywords)
              return item
          }) 
        },
        function4(){
          this.deviceData= this.searchMip(this.MipSearchData);
        },
        searchMip(keywords) {
          return this.deviceData.filter(item =>{
            if(item.mip==keywords)
              return item
          }) 
        },
