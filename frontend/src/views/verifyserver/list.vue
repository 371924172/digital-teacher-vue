...
import {getVerifyserverList,addVerifyserver,getVerifyserver,deleteVerifyserver,updateVerifyserver} from "@/api/verifyserver";
  export default {
    inject: ["reload"],
    data() {
      return {
    ...
    .catch(_ => {});
      ...
      confirm(){
        if(this.addflag==false){
          ...
          this.updateServer(this.editinfo);
        }
        else{//用增加按钮启动该dialog界面
          this.dialogVisible=false;
          //this.serverData.push(this.editinfo);
          this.addServerData(this.editinfo)
        }
          this.serverIndex=0;
          this.addflag=false;
          this.editinfo={
      ...
      getServerData(){//获取数据
        getVerifyserverList().then((response) => {
          this.ServerData = response.data;
          //console.log(this.deviceData)
        });
      },
      addServerData(data){//增
        addVerifyserver(data).then((response) => {
            this.$message({
              message: "添加成功！",
              type: "success",
            });
       ...
       updateServer(data){//更
         updateVerifyserver(data).then((response)=>{
